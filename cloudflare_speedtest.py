...
# 地区扫描缓存文件（按 IP 版本分文件，避免 IPv4/IPv6 缓存相互污染）
REGION_SCAN_FILE = "region_scan.csv"
REGION_SCAN_FILE_V6 = "region_scan_v6.csv"


def get_region_scan_file(ip_file: str) -> str:
    """根据 IP 列表文件路径推导对应的地区扫描缓存文件"""
    if ip_file == CLOUDFLARE_IPV6_FILE:
        return REGION_SCAN_FILE_V6
    return REGION_SCAN_FILE
...
def download_cloudflare_speedtest(os_type, arch_type):
    ...
    if not download_file(download_url, archive_name):
        ...
        if not download_file(http_url, archive_name):
            ...
            print(
                f"解压后文件名应为: CloudflareST_proxy_{os_type}_{arch_type}{'.exe' if os_type == 'win' else ''}"
            )
            print("=" * 60)
        ...
        try:
            ...
            for root, dirs, files in os.walk("."):
                for file in files:
                    if file.startswith("CloudflareST_proxy_") and not file.endswith(
                        (".zip", ".tar.gz")
                    ):
...
def find_airport_by_name(query):
    ...
    if results:
        ...
        try:
            choice = input(
                f"\n请选择 [1-{min(len(results), 10)}] 或按回车取消: "
            ).strip()
            if choice:
                idx = int(choice) - 1
...
def show_main_menu():
    ...
    while True:
        print("\n" + "=" * 70)
        print(" 主菜单 - Cloudflare SpeedTest")
        print("=" * 70)
        print("【1. 测速任务】")
        ...
        if choice == "0":
            ...
            handle_upload_mode_interactive()
        elif choice == "5":
    ...
    if success:
        ...
        while True:
            ...
            if config_choice == "1":
                ...
                while True:
                    ...
                    try:
                        ...
                        if dn_count_int > 200:
                            confirm = (
                                input(
                                    f"  警告: 测试 {dn_count_int} 个IP可能需要较长时间，是否继续？[y/N]: "
                                )
                                .strip()
                                .lower()
                            )
                ...
                print(
                    f"✓ 自定义配置: {dn_count}个IP, {speed_limit}MB/s, {time_limit}ms"
                )
                break
            else:
                print("✗ 无效选择，请输入 1-4")
...
def handle_beginner_mode():
    ...
    print(f"\n🎯 开始测速...")
    print(
        f"参数: 测试{dn_count}个IP, 速度下限{speed_limit}MB/s, 延迟上限{time_limit}ms"
    )
...
def handle_normal_mode():
    ...
    while True:
        ...
        if config_choice == "1":
            ...
            speed_limit = "2"
            time_limit = "500"
            print("✓ 标准测试: 20个IP, 2MB/s, 500ms")
            break
        elif config_choice == "3":
            dn_count = "50"
            speed_limit = "5"
            time_limit = "200"
            print("✓ 高质量测试: 50个IP, 5MB/s, 200ms")
            break
        elif config_choice == "4":
            # 自定义配置
            while True:
    ...
    region_scan_file = get_region_scan_file(ip_file)
    if os.path.exists(region_scan_file):
        print(f"\n正在从扫描结果中提取 {cfcolo} 地区的IP...")

        # 读取该地区的IP
        region_ips = []
        with open(region_scan_file, "r", encoding="utf-8") as f:
        ...
        print(f"❌ 未找到地区扫描结果文件: {region_scan_file}")
...
示例:
  ...
  # 强制重新扫描地区缓存后测速
  python cloudflare_speedtest.py --mode normal --region HKG --rescan
  
  # 优选反代模式
  python cloudflare_speedtest.py --mode proxy --csv result.csv

  # 直接上报已有 result.csv 到 Cloudflare Workers API
  python cloudflare_speedtest.py --mode upload --upload api --worker-domain example.com --uuid abc123

  # 上报前清空远程旧 IP
  python cloudflare_speedtest.py --mode upload --upload api --worker-domain example.com --uuid abc123 --clear

  # 上报到 GitHub 仓库
  python cloudflare_speedtest.py --mode upload --upload github --repo owner/repo --token ghp_xxx
  ...
  python cloudflare_speedtest.py --mode beginner --upload github --repo owner/repo --token ghp_xxx
    ...
    parser.add_argument(
        "--mode",
        choices=["beginner", "normal", "proxy", "upload"],
        required=True,
        help="运行模式: beginner(小白快速测试), normal(常规测速), proxy(优选反代), upload(直接上报已有CSV)",
    )

    # IP版本
    parser.add_argument("--ipv6", action="store_true", help="使用IPv6（默认使用IPv4）")

    # 测试参数
    parser.add_argument("--count", type=int, default=10, help="测试IP数量（默认: 10）")
    parser.add_argument(
        "--speed", type=float, default=1.0, help="下载速度下限 MB/s（默认: 1.0）"
    )
    parser.add_argument(
        "--delay", type=int, default=1000, help="延迟上限 ms（默认: 1000）"
    )
    parser.add_argument(
        "--thread",
        type=int,
    ...
    # 常规测速模式参数
    parser.add_argument(
        "--region", type=str, help="地区码（常规测速模式需要，例如: HKG, SIN）"
    )
    parser.add_argument(
        "--rescan",
        action="store_true",
        help="强制重新扫描所有地区（覆盖 region_scan.csv 缓存）",
    )
    ...
    parser.add_argument(
        "--worker-domain", type=str, help="Worker域名（API上传方式需要）"
    )
    parser.add_argument("--uuid", type=str, help="UUID或路径（API上传方式需要）")
    ...
    parser.add_argument(
        "--token", type=str, help="GitHub Personal Access Token（GitHub上传方式需要）"
    )
    parser.add_argument(
        "--file-path",
        type=str,
        default="cloudflare_ips.txt",
    ...
    parser.add_argument(
        "--clear", action="store_true", help="上传前清空现有IP（避免IP累积，推荐使用）"
    )
...
def run_with_args(args):
    ...
    if args.mode == "beginner":
        ...
        # 确保地区扫描结果可用（没有缓存则自动扫描一次；--rescan 强制重扫）
        available_regions = detect_available_regions(force=args.rescan, ip_file=ip_file)
        if not available_regions:
            print("❌ 地区检测失败，请检查网络")
            return 1

        # 校验用户指定的 --region 是否在扫描结果中
        valid_colos = {r[0] for r in available_regions}
        if args.region not in valid_colos:
            print(f"⚠️  地区 {args.region} 在当前扫描结果中无 IP")
            print(f"   可用地区: {', '.join(sorted(valid_colos))}")
            if not args.rescan:
                print("   提示: 加上 --rescan 参数可强制重新扫描；IPv6 用户加 --ipv6 + --rescan 重新扫 IPv6 段")
            return 1
        ...
        # 直接上报模式：跳过测速，读取已有 CSV
        print(f"\n[直接上报模式]")
        print(f"  CSV文件: {args.csv}")
        print(f"  上传方式: {args.upload}")

        if args.upload == "none":
            print("❌ --mode upload 需要配合 --upload api 或 --upload github 使用")
            return 1

        if not os.path.exists(args.csv):
            print(f"❌ 未找到CSV文件: {args.csv}")
            print("   请先运行测速（--mode beginner/normal）生成结果文件")
            return 1
        ...
        if args.upload == "api":
            ...
            if not args.repo or not args.token:
                ...
                return 1
            if args.clear:
                print("ℹ️  --clear 对 GitHub 模式无效（上传本身会全量替换文件）")
            upload_to_github_cli(
                args.csv,
                args.repo,
                args.token,
        ...
        print("❌ 请指定运行模式: --mode beginner/normal/proxy/upload")
        return 1
...
def save_config(
    worker_domain=None, uuid=None, github_token=None, repo_info=None, file_path=None
):
    """保存配置到文件"""
    try:
        # 加载现有配置
        ...
        if worker_domain and uuid:
            ...
            existing_config["api_last_used"] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )
        ...
        if github_token and repo_info:
            ...
            existing_config["github_last_used"] = datetime.now().strftime(
                "%Y-%m-%d %H:%M:%S"
            )
...
def upload_to_github(result_file="result.csv"):
    ...
    if not github_token or not repo_info:
        ...
        if save_choice not in ["n", "no"]:
            if save_config(
                github_token=github_token, repo_info=repo_info, file_path=file_path
            ):
                print("✅ 配置已保存")
            else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if existing_count > 0:
                    print(f"⚠️  发现已存在 {existing_count} 个优选IP")
                    print(
                        "💡 提示: 使用 --clear 参数可以在上传前清空现有IP，避免IP累积"
                    )
                else:
                    print("✅ 当前无数据，将直接添加")
        except Exception as e:
    ...
    try:
        ...
        try:
            try:
                ...
                error_str = str(e)
                if (
                    "Can't assign requested address" in error_str
...
def handle_rescan_regions():
    ...
    regions = detect_available_regions(force=True, ip_file=ip_file)
    if regions:
        print(f"\n✅ 扫描完成，发现 {len(regions)} 个地区")
    else:
        print("❌ 扫描失败")


def detect_available_regions(force: bool = False, ip_file: str = CLOUDFLARE_IP_FILE):
    """检测可用地区

    Args:
        force: 强制重新扫描，忽略缓存
        ip_file: 扫描使用的 IP 列表文件；决定缓存文件名
            (IPv4→region_scan.csv，IPv6→region_scan_v6.csv)
    """
    region_scan_file = get_region_scan_file(ip_file)

    # 有缓存且非强制重扫 → 直接使用（同时校验文件非空）
    if (
        not force
        and os.path.exists(region_scan_file)
        and os.path.getsize(region_scan_file) > 0
    ):
        # 直接读取已有文件
        available_regions = []
        region_counts = {}
        ...
        total_ips = sum(region_counts.values())
        print(
            f"✓ 使用缓存的地区扫描结果 {region_scan_file}（{len(available_regions)} 个地区，{total_ips} 个 IP）"
        )
    ...
    cmd.extend(
        [
            ...
            "https://jhb.ovh",
            "-o",
            region_scan_file,  # 输出到地区扫描文件
        ]
    )

    try:
        print("运行地区检测...")
        # IPv4 段约 15 个，1-2 分钟；IPv6 段展开后可达 6 万+ IP，需要 10+ 分钟
        scan_hint = "1-2 分钟" if ip_file == CLOUDFLARE_IP_FILE else "10-15 分钟（IPv6 段较多）"
        print(f"正在扫描所有地区，请稍候（约需 {scan_hint}）...")
        print("=" * 50)

        # 直接运行命令，显示完整输出
        # 超时设大一些：IPv6 段 6 万+ IP 需要更长时间，900 秒（15 分钟）兜底
        result = subprocess.run(cmd, timeout=900, encoding="utf-8", errors="replace")

        if result.returncode == 0 and os.path.exists(region_scan_file):
            # 读取检测结果
            available_regions = []
            ...
            # 保留地区扫描结果文件，不删除
            print(f"地区扫描结果已保存到 {region_scan_file}")
